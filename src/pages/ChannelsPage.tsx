...
export const ChannelsModule = () => {
	...
	const ChannelList = () => (
		<div className="glass-panel h-full flex flex-col rounded-2xl overflow-hidden">
			<div className="p-4 border-b border-slate-200 dark:border-white/5 bg-white/50 dark:bg-slate-900/50 backdrop-blur sticky top-0 z-10">
				<div className="flex justify-between items-center mb-4">
					<h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2"><Megaphone size={24} className="text-indigo-400" /> Channels</h2>
				...
				<div className="relative">
					...
					<input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search channels..." className="w-full bg-slate-100 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 transition-colors placeholder:text-slate-500 dark:placeholder:text-slate-600" />
				</div>
			</div>
			<div className="flex-1 overflow-y-auto p-2 space-y-2 no-scrollbar pb-24 md:pb-2">
				{filteredChannels.map(channel => (
					<div key={channel.id} onClick={() => setSelectedChannel(channel)} className="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-100 dark:hover:bg-white/5 cursor-pointer transition-colors group">
						<div className="h-12 w-12 rounded-xl bg-gradient-to-br from-indigo-600 to-emerald-600 flex items-center justify-center text-white font-bold text-lg shrink-0 shadow-lg border border-slate-200 dark:border-white/10 group-hover:scale-105 transition-transform">
							{channel.name[0]}
						</div>
						<div className="flex-1 min-w-0">
							<div className="flex justify-between items-center">
								<h4 className="text-slate-900 dark:text-white font-bold truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-200 transition-colors">{channel.name}</h4>
								{channel.isOwner && <span className="text-[10px] bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border border-indigo-500/30 px-1.5 py-0.5 rounded font-bold uppercase">Admin</span>}
							</div>
							<p className="text-xs text-slate-500 dark:text-slate-400 truncate group-hover:text-slate-600 dark:group-hover:text-slate-300">{channel.description}</p>
						</div>
					</div>
				))}
	...
	const ChannelFeed = () => {
		...
		return (
			<div className="glass-panel h-full flex flex-col rounded-2xl overflow-hidden relative">
				<div className="p-3 border-b border-slate-200 dark:border-white/5 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl flex justify-between items-center sticky top-0 z-20">
					<div className="flex items-center gap-2 min-w-0">
						...
						>
							<div className="h-10 w-10 rounded-lg bg-gradient-to-br from-indigo-600 to-emerald-600 flex items-center justify-center text-white font-bold flex-shrink-0 shadow-md border border-slate-200 dark:border-white/10">
								{selectedChannel.name[0]}
							</div>
							<div className="min-w-0 max-w-[150px] sm:max-w-xs">
								<h3 className="text-slate-900 dark:text-white font-bold truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-200 transition-colors">{selectedChannel.name}</h3>
								<p className="text-xs text-slate-500 dark:text-slate-400">{selectedChannel.subscribers.toLocaleString()} subscribers</p>
							</div>
						</div>
					</div>
					<div className="flex gap-1 flex-shrink-0">
						<Button size="icon" variant="ghost" className="rounded-full hover:bg-slate-100 dark:hover:bg-white/10 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"><Search size={20} /></Button>
						<Button size="icon" variant="ghost" onClick={() => setShowInfoModal(true)} className="rounded-full hover:bg-slate-100 dark:hover:bg-white/10 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white"><MoreVertical size={20} /></Button>
					</div>
				</div>

				<div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 no-scrollbar min-h-0 bg-slate-50 dark:bg-black/20 pb-24 md:pb-6">
					{selectedChannel.posts.map((post: any) => (
						<div key={post.id} className="glass-card p-5 max-w-2xl mx-auto shadow-lg border-slate-200 dark:border-white/5 animate-in fade-in slide-in-from-bottom-2 duration-500">
							<p className="text-slate-900 dark:text-slate-100 text-sm leading-relaxed whitespace-pre-wrap font-medium">{post.text}</p>
							<div className="mt-4 flex justify-between items-center text-slate-500 text-xs border-t border-slate-200 dark:border-white/5 pt-3">
				...
				{selectedChannel.isOwner ? (
					<div className="p-4 border-t border-slate-200 dark:border-white/5 bg-white/90 dark:bg-slate-900/90 backdrop-blur sticky bottom-20 md:bottom-0 z-20 flex-shrink-0">
						<div className="flex gap-3 items-end max-w-3xl mx-auto">
							<textarea
								value={broadcastText}
								onChange={e => setBroadcastText(e.target.value)}
								placeholder="Broadcast a message..."
								className="flex-1 bg-slate-100 dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-2xl p-4 text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none text-sm h-14 max-h-32 transition-colors"
							/>
