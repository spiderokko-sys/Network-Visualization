...
export const ChatProfileModal = ({ chat, onClose }) => {
	...
	return (
		...
		>
			<div
				className="w-full max-w-sm bg-white dark:bg-slate-900 h-full border-l border-slate-200 dark:border-slate-800 shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-300"
			...
			>
				<div className="relative">
					...
					{showMenu && (
						<div className="absolute top-16 right-4 w-48 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-40 overflow-hidden animate-in fade-in zoom-in-95">
							{chat.isGroup ? (
								<>
									<button className="w-full text-left px-4 py-3 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center gap-2 transition-colors">
							...
							<div className="h-px bg-slate-200 dark:bg-slate-700 my-0"></div>
					...
					<div className="absolute -bottom-12 left-6">
						<div className="h-24 w-24 rounded-full bg-slate-100 dark:bg-slate-800 p-1 shadow-xl">
				...
				<div className="mt-14 px-6 pb-6 border-b border-slate-200 dark:border-slate-800">
				...
				<div className="p-4 space-y-1">
					{!chat.isGroup && (
						<>
							<div className="flex items-center gap-4 p-3 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl cursor-pointer transition-colors">
								<Phone size={20} className="text-slate-500 dark:text-slate-400" />
								<div>
									<div className="text-slate-900 dark:text-white text-sm">+1 (416) 555-0192</div>
							...
							<div className="flex items-center gap-4 p-3 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl cursor-pointer transition-colors">
								...
								<div>
									<div className="text-slate-900 dark:text-white text-sm">contact@gigmind.net</div>
					...
					<div className="flex items-center gap-4 p-3 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl cursor-pointer transition-colors">
						...
						<div className="flex-1">
							<div className="text-slate-900 dark:text-white text-sm">Notifications</div>
				...
				{chat.isGroup && (
					<>
						...
						<div className="p-4">
							<h3 className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase mb-3">Group Members <span className="text-slate-400 dark:text-slate-600 font-normal">({groupMembers.length})</span></h3>
							<div className="space-y-3">
								{groupMembers.map(member => (
									<div key={member.id} className="flex items-center justify-between group cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/50 p-2 rounded-lg transition-colors">
										<div className="flex items-center gap-3">
											<div className="h-8 w-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-700 dark:text-white">
												{member.name[0]}
											</div>
											<div>
												<div className="text-sm text-slate-900 dark:text-white font-medium">{member.name}</div>
					...
					<div className="flex border-b border-slate-200 dark:border-slate-800 mb-4">
						{['media', 'attachments', 'links'].map(tab => (
							<button
								...
								className={`flex-1 pb-2 text-xs font-bold uppercase tracking-wider text-center transition-colors ${activeTab === tab
					...
					<div className="min-h-[150px]">
						{activeTab === 'media' && (
							<div className="grid grid-cols-4 gap-2 animate-in fade-in">
								{mediaItems.map(i => (
									<div key={i} className="aspect-square bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400 dark:text-slate-600 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer">
										<ImageIcon size={16} />
									</div>
								))}
							</div>
						...
						{activeTab === 'attachments' && (
							<div className="space-y-2 animate-in fade-in">
								{attachmentItems.map(file => (
									<div key={file.id} className="flex items-center gap-3 p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg cursor-pointer group">
										<div className="p-2 bg-slate-100 dark:bg-slate-800 rounded text-slate-600 dark:text-slate-400 group-hover:bg-slate-200 dark:group-hover:bg-slate-700 group-hover:text-indigo-400 transition-colors">
											<FileText size={18} />
										</div>
										<div className="flex-1 min-w-0">
											<div className="text-xs text-slate-900 dark:text-white font-medium truncate">{file.name}</div>
											<div className="text-[10px] text-slate-500">{file.size}</div>
										</div>
										<Download size={14} className="text-slate-400 dark:text-slate-600 group-hover:text-white" />
									</div>
								))}
							</div>
						)}
						...
						{activeTab === 'links' && (
							<div className="space-y-2 animate-in fade-in">
								{linkItems.map(link => (
									<div key={link.id} className="flex items-center gap-3 p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg cursor-pointer group">
										<div className="p-2 bg-slate-100 dark:bg-slate-800 rounded text-slate-600 dark:text-slate-400 group-hover:bg-slate-200 dark:group-hover:bg-slate-700 group-hover:text-emerald-400 transition-colors">
											<LinkIcon size={18} />
										</div>
										<div className="flex-1 min-w-0">
											<div className="text-xs text-slate-900 dark:text-white font-medium truncate">{link.title}</div>
											<div className="text-[10px] text-slate-500 truncate">{link.url}</div>
										</div>
										<ChevronRight size={14} className="text-slate-400 dark:text-slate-600 group-hover:text-white" />
									</div>
								))}
							</div>
						)}
